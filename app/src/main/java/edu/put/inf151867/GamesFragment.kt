...
import edu.put.inf151867.databinding.FragmentGamesBinding
import java.net.URL
...
class GamesFragment : Fragment() {
    ...
    private fun showGames(sort: String?) {
        val table = binding.gamesTable
        table.removeAllViews()
        val dbHandler = activity?.let { MyDBHandler(it.applicationContext, null, null, 1) }
        val gamesList: MutableList<Game>? = dbHandler?.findGames("boardgame",sort)
        var id = 1
        ...
        if (gamesList != null) {
            for (game in gamesList) {
                ...
                tableRow.setOnClickListener {
                    ...
                    findNavController().navigate(R.id.action_GamesFragment_to_gameDetailsFragment, bundle)
                }
